...
def load_for_recommend(args, user_history_dict):
    n_entity, n_relation, kg = load_kg(args)
    ripple_set = get_ripple_set(args, kg, user_history_dict)
    return n_entity, n_relation, ripple_set
...
def load_rating(args):
    ...
    rating_file = PATH_MOVIEDB + 'ratings_final'
    if os.path.exists(rating_file + '.npy'):
        rating_np = np.load(rating_file + '.npy')
    else:
        rating_np = np.loadtxt(rating_file + '.txt', delimiter=',', dtype=np.int32)
...
def get_ripple_set(args, kg, user_history_dict):
    ...
    print(len(set(user_history_dict.keys())))

...
@logger('Initialize the RippleNet model:', ('n_entity', 'n_relation', 'hop_size', 'ripple_size', 'dim', 'kge_weight', 'l2', 'item_update_mode'))
def RippleNet_model(n_entity: int, n_relation: int, ripple_set: list, hop_size=2, ripple_size=32, dim=16,
    ...
    kge_loss = 0  # Knowledge graph embedding loss term
    for hop in range(hop_size):
    ...
    l2_loss = 0  # The additional l2 regularization term significantly improves the auc index of RippleNet on the ml1m data set.
    for hop in range(hop_size):
